...
import { ImageIcon, Plus, Search, Pencil, Trash2 } from "lucide-react";
import { IMAGE_ACCEPT, uploadPublicImage, validateImageFile } from "@/lib/media";
...
function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return (products.data ?? []).filter((p: any) => {
      ...
      if (filter === "low")
        return Number(p.stock_quantity) <= Number(p.min_stock) && Number(p.stock_quantity) > 0;
  ...
  const save = useMutation({
    mutationFn: async (p: Product) => {
      const image_url = imageFile
        ? await uploadPublicImage(imageFile, "products")
        : p.image_url || null;
              ...
              <TableBody>
                ...
                {filtered.map((p: any) => {
                  ...
                  return (
                    <TableRow
                      key={p.id}
                      className={low ? "bg-amber-50/40" : out ? "bg-destructive/5" : ""}
                    >
                      <TableCell>
                        <div className="flex items-center gap-3">
                          {p.image_url ? (
                            ...
                            <div className="grid h-11 w-11 place-items-center rounded-xl border bg-muted text-muted-foreground">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <Field label="Supplier">
                <Select
                  ...
                  onValueChange={(v) =>
                    setEditing({ ...editing, supplier_id: v === "none" ? null : v })
                  }
                >
                  <SelectTrigger>
                    <SelectValue placeholder="None" />
              ...
              <div className="space-y-2 sm:col-span-2">
                <Label className="text-xs">Product image / WebP supported</Label>

                <div className="flex flex-col gap-3 rounded-2xl border border-dashed border-border bg-muted/25 p-3 sm:flex-row sm:items-center">
                  {imagePreview ? (
                    <img
                      src={imagePreview}
                      alt="Product preview"
                    ...
                    <div className="grid h-20 w-20 place-items-center rounded-xl border bg-white text-muted-foreground">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Allowed: JPG, PNG, WebP. Recommended: compressed WebP under 5MB for faster
                      loading.
                    </p>
                  </div>
              ...
              <Field label={`Selling price (${sym})`}>
                <Input
                  type="number"
                  step="0.01"
                  value={editing.selling_price}
                  onChange={(e) =>
                    setEditing({ ...editing, selling_price: Number(e.target.value) })
                  }
                />
              </Field>
